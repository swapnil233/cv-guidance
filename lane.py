...
filename = "test_videos/test3.mp4"
...
class Lane:
    ...
    def __init__(self, orig_frame):
        ...
        self.roi_points = np.float32(
            [
                (int(0.456 * width), int(0.644 * height)),  # Top-left corner
                (0, height - 1),  # Bottom-left corner
                (int(0.958 * width), height - 1),  # Bottom-right corner
                (int(0.6183 * width), int(0.644 * height)),  # Top-right corner
            ]
        )
    ...
    def calculate_histogram(self, frame=None, plot=True):
        ...
        if plot == True:
            ...
            ax1.set_title("Bird's Eye View of Lane Lines")
    ...
    def display_curvature_offset(self, frame=None, plot=False):
        ...
        text_message = f"{abs(self.center_offset) / 100:.2f} m {direction}"
        ...
        cv2.putText(
            ...
            text_message,
            (
                int((5 / 600) * self.width),
                int((20 / 338) * self.height),
            ),  # position of the text
            cv2.FONT_HERSHEY_SIMPLEX,  # font
            (float((0.5 / 600) * self.width)),  # font scale
    ...
    def overlay_lane_lines(self, plot=False):
        ...
        # Calculate and draw the center of the lane
        # Calculate midpoint of the left and right lanes for each y coordinate
        center_fitx = (self.left_fitx + self.right_fitx) / 2
        # Prepare points to draw the center lane line
        pts_center = np.array([np.transpose(np.vstack([center_fitx, self.ploty]))])
...
def main():
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            # Add the following to display the edge-detected image
            # cv2.imshow("Edge Detected Frame", lane_line_markings)
